    ...
    public class POEModel
    {
        ...
        public List<Character> GetCharacters()
        {
            ...
            foreach (var character in characters){
                HashSet<ushort> passives = GetPassiveSkills(character.Name);
                chars.Add(new Character(character) { PassiveSkills = passives });
            }
            return chars;
        }
        ...
        public HashSet<ushort> GetPassiveSkills(string characterName)
        {
            try
            {
                if (downOnlyMyCharacters && !Settings.Lists["MyCharacters"].Contains(characterName))
                ...
                DataContractJsonSerializer serialiser = new DataContractJsonSerializer(typeof(JSONProxy.CharacterPassives));
                JSONProxy.CharacterPassives passives;

                using (Stream stream = transport.GetPassiveSkills(characterName))
                    passives = (JSONProxy.CharacterPassives)serialiser.ReadObject(stream);

                if (passives.Passives == null)
                    return new HashSet<ushort>();
            ...
            {
                Logger.Log(sex);
                throw new Exception(string.Format("Error reading character passive skills for {0}, if you are in offline mode you will need to login and update. If you received this error while logging in, the authenticated session may have expired or bad data has been returned by GGG or a network issue may have occurred - Please try again.", characterName));
            }
        }
        ...
        public void GetImages(List<Item> items)
        {
            foreach (var item in items.Distinct(new ImageComparer()))
                if (item.ItemType == ItemType.Gear) //My modification to only get image of items
                    getImageWithEvents(item);
        }
